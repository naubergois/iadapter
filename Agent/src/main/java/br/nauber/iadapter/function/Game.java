...
public class Game {
	...
	public static void setDefenderConfiguration(
	...
	public static void init(List<TestGene> sampleGenesAtackList,
			List<TestGene> sampleGenesDefendList, int populationSizeAtack,
			int populationSizeDefender) throws InvalidConfigurationException {
		...
		for (TestGene gene : sampleGenesDefendList) {
			...
			if (gene instanceof ScriptGene) {
				sampleGenesDefend[counter] = new IntegerGene(conf, 0,
						((ScriptGene) gene).getScriptName());
				counter++;
				sampleGenesDefend[counter] = new IntegerGene(conf, 0,
						((ScriptGene) gene).getUserNumber());
				counter++;
			}
		...
		for (TestGene gene : sampleGenesAtackList) {
			if (gene instanceof IntGene) {
				sampleGenesAtack = new Gene[sampleGenesAtackList.size()];
				sampleGenesAtack[counter] = new IntegerGene(conf,
						((IntGene) gene).getMin(), ((IntGene) gene).getMax());
				counter++;
			}
			if (gene instanceof ScriptGene) {
				sampleGenesAtack = new Gene[sampleGenesAtackList.size() * 2];
				sampleGenesAtack[counter] = new IntegerGene(conf, 0,
						((ScriptGene) gene).getScriptName());
				counter++;
				sampleGenesAtack[counter] = new IntegerGene(conf, 0,
						((ScriptGene) gene).getUserNumber());
